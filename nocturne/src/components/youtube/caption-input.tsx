...
export function CaptionInput({ keywords, onFlashcards }: Props) {
  ...
  return (
    <div className="pt-3 border-t border-border-default">
      ...
      <div className="flex gap-1.5">
        <input
          ...
          className={[
            ...
            error
              ? 'border-rose-400 focus:border-rose-400 shadow-red'
              : 'border-border-default focus:border-indigo-500 focus:shadow-teal',
          ].join(' ')}
      ...
      {error && (
        <p className="text-caption text-rose-300 mt-1">{error}</p>
      )}
      {successCount !== null && !error && (
        <p className="text-caption text-indigo-400 mt-1">
