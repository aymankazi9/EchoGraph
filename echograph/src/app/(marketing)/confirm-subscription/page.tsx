...
export const metadata: Metadata = {
  title: 'Confirm subscription — EchoGraph',
}
...
export default async function ConfirmSubscriptionPage({
  ...
  if (!token) {
    return (
      <PageFade>
        <Centered>
          <AlertTriangle size={40} strokeWidth={1.25} className="text-red-200 mb-5" />
          ...
          <Link
            href="/#newsletter"
            className="inline-flex h-9 px-5 items-center rounded-btn text-body-sm font-medium bg-teal-300 text-text-inverse hover:bg-teal-400 transition-colors"
          >
            Subscribe again
          </Link>
        </Centered>
  ...
  if (result.status === 'confirmed') {
    return (
      <PageFade>
        <Centered>
          <CheckCircle size={40} strokeWidth={1.25} className="text-teal-300 mb-5" />
          <h1 className="text-subheading font-medium text-text-primary mb-3">
            You&apos;re subscribed.
          </h1>
          <p className="text-body-sm text-text-secondary mb-8">
    ...
    return (
      <PageFade>
        <Centered>
          ...
          <p className="text-body-sm text-text-secondary mb-8">
            This email is already subscribed to EchoGraph updates.
          </p>
          <Link
            href="/"
            className="inline-flex h-9 px-4 items-center rounded-btn text-body-sm text-text-secondary border border-border-default hover:bg-bg-subtle transition-colors"
