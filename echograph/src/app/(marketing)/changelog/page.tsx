...
export const metadata: Metadata = {
  title: 'Changelog — EchoGraph',
  description: 'What\'s new in EchoGraph.',
}
          ...
          {entries.map((entry) => (
            <div key={entry.version}>
              <div className="flex items-center gap-3 mb-5">
                ...
                <span className="text-caption uppercase tracking-[0.07em] px-2 py-0.5 rounded-full bg-purple-500/20 text-purple-200 border border-purple-500/30">
                  {entry.label}
                </span>
                ...
                <ul className="flex flex-col gap-2.5">
                  {entry.items.map((item) => (
                    <li key={item} className="flex items-start gap-2.5">
                      <Check size={13} strokeWidth={2} className="text-teal-300 mt-0.5 shrink-0" />
