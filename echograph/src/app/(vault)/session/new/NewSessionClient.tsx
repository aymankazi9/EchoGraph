    ...
    <div className="flex items-center gap-2 text-label">
      <span
        className={[
          ...
          mode === 'local' ? 'bg-teal-300 animate-pulse-slow' : 'bg-amber-200',
        ].join(' ')}
      />
          ...
          <button
            ...
            className={[
              ...
              hasFiles
                ...
                : 'bg-teal-300 text-text-inverse opacity-30 cursor-not-allowed',
            ].join(' ')}
          >
            Start ingestion
          </button>
      ...
      {phase === 'done' && (
        <motion.div variants={fadeUp} initial="hidden" animate="visible">
          <button
            type="button"
            onClick={() => router.push('/dashboard')}
            className="h-9 px-4 rounded-btn text-body font-medium bg-teal-300 text-text-inverse hover:bg-teal-400 transition-colors"
          >
            Go to dashboard →
          </button>
        </motion.div>
