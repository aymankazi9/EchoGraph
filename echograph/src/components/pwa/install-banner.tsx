...
const DISMISSED_KEY = 'echograph-install-dismissed'
...
export function InstallBanner({ navOffset = false }: Props) {
  ...
  useEffect(() => {
    ...
    if (navOffset) {
      const collapsed = localStorage.getItem('echograph-nav-collapsed') === 'true'
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center gap-2.5">
        <Download size={16} strokeWidth={1.5} className="text-teal-300 shrink-0" />
        <span className="text-body-sm font-medium text-text-primary">Install EchoGraph</span>
      ...
      <div className="flex items-center gap-2">
        <button
          ...
          className="h-8 px-3.5 rounded-btn text-[13px] font-medium bg-teal-300 text-text-inverse hover:bg-teal-400 transition-colors"
        >
          Install
        </button>
        <button
